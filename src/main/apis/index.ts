...
import { LyricResponse, ClientEvent } from '../types';
...
export const getLyricRes = async (data: Info): Promise<LyricResponse[]> => {
  const lyricRes = await Promise.all([searchKugou(data), search163(data), searchLrclib(data)]);
...
export const setPlayer = async (win: BrowserWindow, data: ClientEvent.Player) => {
