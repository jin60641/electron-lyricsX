...
const searchLyric: (info: KugouLyricRequest) => Promise<LyricResponse> = async ({
  ...
  const res = await axios.get<KugouLyricResponse>('http://lyrics.kugou.com/download', {
    params: { ver: 1, client: 'pc', ...info, accesskey },
  });
...
const searchMusic = async (data: LyricRequest): Promise<LyricResponse[]> => {
  const res = await axios
    .get<KugouSearchResponse>('http://lyrics.kugou.com/search', {
      params: {
        offset: 0,
        limit: 10,
        ver: 1,
  ...
  const infos: KugouLyricRequest[] = songs.map(({ id, accesskey, song: name, singer: artist }) => ({
    id,
    name,
    artist,
    accesskey,
