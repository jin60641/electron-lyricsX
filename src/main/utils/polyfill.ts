...
// ease out quint
const DEFAULT_EASING = (index: number, current: number, diff: number, duration: number) =>
  index === duration ? current + diff : diff * (1 - (1 - index / duration) ** 5) + current;
...
BrowserWindow.prototype.setBounds = function setBounds(
  ...
  const { duration = DEFAULT_DURATION, easing = DEFAULT_EASING } =
  ...
  const updateBounds = (currentFrame: number) => {
    const eased = Object.keys(targetBounds).reduce((obj, k) => {
      ...
      return {
        ...obj,
        [key]: Math.round(easing(currentFrame, currentBounds[key], diff, duration)),
      };
    }, targetBounds);
