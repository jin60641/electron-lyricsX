...
const getHash = (msg: string, key: string) => {
  const hash: Crypto.Hmac = Crypto.createHmac('md5', key).update(msg);
  return hash.digest('base64');
};

const C_TYPE: string = 'application/x-www-form-urlencoded; charset=UTF-8'; // Content-Type
const UA: string =
  'Mozilla/5.0 (Macintosh Intel Mac OS X 11_1_0) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/87.0.4280.67 Safari/537.36'; // User-Agent
...
export const getConfig = async (url: string) => {
  ...
  const mainJsUrl = Array.from($naver('script').map((_i, script) => script.attribs.src)).find(
    (src) => /\/main.(.*).chunk.js/.test(src),
  );
...
export const request = async <T>(
  ...
  const body = new URLSearchParams(Object.entries(data).map(([k, v]) => [k, v.toString()]));
