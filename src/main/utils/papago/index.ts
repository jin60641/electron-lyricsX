...
export const detectLanguage = async (query: string) => {
  ...
  const resp = await request<{ langCode: string }>(url, { query });
...
export const tlit = async (params: TlitParameter) => {
  const srcLang =
    !params.srcLang || params.srcLang === 'detect'
      ? await detectLanguage(params.query)
      : params.srcLang;
  ...
  const resp = await request<TlitResponse>(url, { ...params, srcLang });
  return resp;
};
...
export const Translator = async (params: TranslateParameter) => {
  const source =
    !params.source || params.source === 'detect'
      ? await detectLanguage(params.text)
      : params.source;
