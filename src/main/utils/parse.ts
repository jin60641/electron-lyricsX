...
import { KrcRow, KrcWord, LrcRow, LyricFormat, Music, Row } from '../../types';
import { LyricResponse } from '../types';

import { tlit } from './papago';
import { filterRegex, krcLineRegex, krcTimeTagRegex, krcWordRegex, lrcTimeTagRegex } from './regex';
...
const addFuriganaText = (text: string) =>
...
const parseWords = async (str: string): Promise<KrcWord[]> => {
  ...
  const words = await Promise.all(
    matches.map(async ([_sub, _offset, duration, _, text]) => ({
...
export const parseRowData = async (filteredLyrics: LyricResponse[]) => {
  const lyrics = await Promise.all(
    filteredLyrics.map(async ({ lyric, ...info }) => {
...
export const KRC_ENCODE_KEY = Buffer.from(
  new Uint8Array([64, 71, 97, 119, 94, 50, 116, 71, 81, 54, 49, 45, 206, 210, 110, 105]),
);
...
export const tlitLyric = async ({ lyric, locale }: { lyric: Music['lyric']; locale: string }) => {
  ...
  const promises = srcTexts.map(async (query) =>
    tlit({
      query,
      tlitLang: locale,
    }),
  ...
  const tlits = await Promise.all(promises).then((items) =>
    items.map((item) => item?.message?.tlitResult).flat(),
  );
